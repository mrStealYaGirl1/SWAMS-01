...
int main(void)
{
	...
	while (1)
	{
		...
		if (NRF24SendPacket(payload, 4)) {
			...
			UART_SendHex(NRF24GetStatus());
			UART_SendString("\r\n");
			UART_SendString("CONFIG=");
			UART_SendHex(NRF24ReadRegister(0x00));
			UART_SendString("\r\n");

			UART_SendString("RF_CH=");
			UART_SendHex(NRF24ReadRegister(0x05));
			UART_SendString("\r\n");

			UART_SendString("RF_SETUP=");
			UART_SendHex(NRF24ReadRegister(0x06));
			UART_SendString("\r\n");
